...
export async function generatePanorama({
  ...
  // Optional small preview (data URI) for showing the result in a browser
  // without transferring/rendering the full-size image.
  let preview = null;
  if (previewWidth > 0) {
    const pw = Math.min(previewWidth, outW);
    const previewBuf = await sharp(buf, { limitInputPixels: false })
      .resize(pw)
      .jpeg({ quality: 68 })
      .toBuffer();
